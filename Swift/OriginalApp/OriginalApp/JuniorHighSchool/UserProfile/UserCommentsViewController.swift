...
class UserCommentsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet var userCommentsTableView: UITableView!
    

    override func viewDidLoad() {
        super.viewDidLoad()

        userCommentsTableView.delegate = self
        userCommentsTableView.dataSource = self
        
        let nib = UINib(nibName: "userAnswerTableViewCell", bundle: Bundle.main)
        userCommentsTableView.register(nib, forCellReuseIdentifier: "Cell")
        
        userCommentsTableView.rowHeight = 272
        
        userCommentsTableView.tableFooterView = UIView()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell") as! userAnswerTableViewCell
        ...
        createDateLabel.text = DateUtils3.stringFromDate(date: comments[indexPath.row].createDate, format: "yyyy年MM月dd日 HH時mm分")
                            ...
                            let deleteAction = UIAlertAction(title: "OK", style: .default) { (acrion) in
                                post?.deleteInBackground({ (error) in
                                    if error != nil {
                                        ...
                                        tableView.deselectRow(at: indexPath, animated: true)
                                        self.loadComments()
                                        self.userCommentsTableView.reloadData()
                                    }
                                })
                            }
                            alertController.addAction(cancelAction)
